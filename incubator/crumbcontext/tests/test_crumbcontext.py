...
def test_extracts_and_sanitizes_exact_values():
    text = "Deploy /workspace/app.py at 2026-07-11, SHA abcdef1234567890, cost CAD $14,360.00; email dev@example.com"
...
def test_authority_and_recency_stay_exact():
    blocks = [
        ContextBlock(id="system", role="system", kind="instruction", content="x" * 9000, age_turns=99),
        ContextBlock(id="now", role="user", kind="message", content="x" * 9000, age_turns=0),
    ]
...
def test_old_dense_context_routes_to_image():
    content = "\n".join('{"path":"/tmp/a.py","status":"ok","value":12345678}' for _ in range(400))
    block = ContextBlock(id="old", role="user", kind="tool_result", content=content, age_turns=8)
...
def test_memory_prefers_cache_when_reused():
    block = ContextBlock(id="mem", role="user", kind="memory", content="decision line\n" * 300, age_turns=10, reuse_count=4)
    assert route_blocks([block]).blocks[0].lane is Lane.CACHE


def test_demo_writes_shareable_artifacts(tmp_path: Path):
    blocks = [ContextBlock.from_dict(item, i) for i, item in enumerate(demo_payload()["blocks"])]
